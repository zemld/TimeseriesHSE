...
class DatabaseManager:
    ...
    async def insert_data(self, table_name: str, data: dict) -> None:
        await self._check_and_create_connetion()
        insert_query = f"INSERT INTO {table_name} (datetime, price) VALUES\n\t"
        values = ",\n\t".join(
            [
                f"('{datetime}', {price if price is not None else "null"})"
                for datetime, price in data.items()
            ]
        )
    ...
    async def delete_data(self, table_name: str, till_date: datetime) -> None:
        await self._check_and_create_connetion()
