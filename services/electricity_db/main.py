...
db_manager = FastAPI()
db = DatabaseManager("electricity_db", 5433, "electricity_db", "user", "secret")
logger = Logger("electricity_db_service")
...
async def select_data(table_name: str, from_datetime: str, till_datetime: str):
    logger.info(f"Recieved request to select data from table {table_name}")
    try:
        data = await db.select_data(table_name, from_datetime, till_datetime)
        return {"data": data}
    except Exception as e:
